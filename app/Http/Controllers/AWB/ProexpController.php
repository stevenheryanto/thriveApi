...
use App\Models\AWB\awb_trn_proexp;

class ProexpController extends Controller
{
    protected $table_name = 'awb_trn_proexp';
    ...
    public function ListData(Request $request)
    {
		...
		$sql = "
            select a.id,
                a.name,
                a.description,
                ifnull(b.full_name,b.name) as emp_name, 
                a.status
            from " .  $this->table_name ." a 
                inner join users b on a.user_created = b.id 
            where 
                1=1                 
    ...
    function ListMenu(Request $request)
	{
		...
		$sql = "
				...
				from 
                    awb_mst_menu a 
                    left join awb_trn_proexp b on a.section_id = b.id 
				where 
                    a.flag_active = 1
    ...
    function ListCategory(Request $request)
	{
		...
		$sql = "
            ...
            FROM 
                awb_trn_category a, awb_mst_menu b, awb_trn_proexp c
            WHERE 
                a.menu_id = b.id
