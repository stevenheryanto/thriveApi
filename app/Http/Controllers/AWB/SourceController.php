...
use App\Models\AWB\awb_mst_sources;

class SourceController extends Controller
{
    
    protected $folder_name  = 'learn/sources';
    protected $table_name   = 'awb_mst_sources';
    ...
    public function ListData(Request $request)
    {
        ...
        try {
            ...
            $query = awb_mst_sources::where('platform_id','=',$platform_id);
    ...
    public function InsertData(Request $request)
    {
        ...
        $file       = $request->file('sources_image');
        ...
        unset($_arrayData['user_account'] , $_arrayData['sources_image'], $_arrayData['user_id']);

        $_arrayData = array_merge($_arrayData,
        array(
            'sources_image' => $fileName,
    ...
    public function UpdateData(Request $request)
    {
        $id         = $request->input('id');
        $user_id    = $request->input('user_id');
        $all        = $request->except('user_account','id','sources_image','user_id');

        if($request->hasFile('sources_image'))
        {
            $file       = $request->file('sources_image');
            $fileName   = $request->input('user_account'). '_' .$file->getClientOriginalName();
            $fileName   = DB_global::cleanFileName($fileName);
    ...
    public function SelectData(Request $request)
	{
        ...
        try {
            ...
            $query =  awb_mst_sources::where(DB::RAW('md5(id)'), '=', $id);
            $data = $query->first();
