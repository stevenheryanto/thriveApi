...
use DB_global;
use App\Models\Findtime\ideation_platform_hdr;
...
{
    protected $table_name = 'ideation_platform_hdr';
    protected $folder_name = 'ideation/platform';
    ...
    public function ListData(Request $request)
    {
        ...
        try {
            $query = ideation_platform_hdr::select('*');
            if($category != "COUNT"){
   ...
   public function SelectData(Request $request)
   {
       ...
       $sqlDtl1 = "select * from ideation_platform_dtl_1 where md5(platform_id) = ?";
       $sqlDtl2 = "select * from ideation_platform_dtl_2 where md5(platform_id) = ?";
       $sqlDtl3 = "select a.id, a.account, a.name from users a left join ideation_platform_dtl_4 b on b.user_id = a.id where md5(b.platform_id) = ? and group_id = 1";
       $sqlDtl4 = "select a.id, a.account, a.name from users a
       left join ideation_platform_dtl_3 b on b.imdl_id = a.id where md5(b.platform_id) = ?";
       $sqlDtl5 = "select a.id, a.account, a.name from users a left join ideation_platform_dtl_4 b on b.user_id = a.id where group_id = 2";
   ...
   public function InsertData(Request $request)
   {
        ...
        try {
            /* insert to $this->table_name and get id */
            $platform_id = DB_global::cz_insert($this->table_name, $_arrayData, true);

            /* loop array then insert ideation_platform_dtl_1 */
            if(count($arrCountry) > 0){
                foreach($arrCountry as $countryDtl){
                    ...
                    $dtl1 = DB_global::cz_insert('ideation_platform_dtl_1', $_arrayDtl1, true);
                }
            }
            ...
            if(count($arrDirectorate) > 0){
                foreach($arrDirectorate as $directorateDtl){
                    ...
                    $dtl1 = DB_global::cz_insert('ideation_platform_dtl_2', $_arrayDtl2, true);
                }
            }
            ...
            if(count($arrAdhoc) > 0){
                foreach($arrAdhoc as $adhocDtl){
                    ...
                    $dtl1 = DB_global::cz_insert('ideation_platform_dtl_3', $_arrayDtl4, true);
                }
            }
   ...
   public function UpdateData(Request $request)
    {
        ...
        try {
            ...
            if(count($arrCountry) > 0){
                ...
                $this->UpdateFlag('ideation_platform_dtl_1', $platform_id);
                $paramDel = array(
                    'platform_id' => $platform_id,
                    'flag_active' => 0
                );
                DB_global::cz_delete_array('ideation_platform_dtl_1', $paramDel);
                foreach($arrCountry as $countryDtl){
                    $sql = "SELECT id FROM ideation_platform_dtl_1 WHERE platform_id=:platform_id and country=:country";
                    $param = array(
                        'platform_id' => $platform_id,
                        'country' => $countryDtl->value,
                    );
                    ...
                    if(!empty($checkCountry)){
                        ...
                        DB_global::cz_update('ideation_platform_dtl_1', 'flag_active', 1, $param);
                    } else {
            ...
            if(count($arrDirectorate) > 0){
                /* update flag_active to 0 */
                $this->UpdateFlag('ideation_platform_dtl_2', $platform_id);
                $paramDel = array(
                    'platform_id' => $platform_id,
                    'flag_active' => 0
                );
                DB_global::cz_delete_array('ideation_platform_dtl_2', $paramDel);
                foreach($arrDirectorate as $directorateDtl){
                    $sql = "SELECT id FROM ideation_platform_dtl_2 WHERE platform_id=:platform_id and 'directorate'=:directorate";
                    $param = array(
                        'platform_id' => $platform_id,
                        'directorate' => $directorateDtl->value,
                    );
                    ...
                    if(!empty($checkFunction)){
                        /* kalo ada, update flag ke 1*/
                        DB_global::cz_update('ideation_platform_dtl_2', 'flag_active', 1, $param);
                    } else {
                        $_arrayDtl2 = array(
                            'platform_id' => $platform_id,
                            'directorate' => $directorateDtl->value,
                        ...
                        $dtl2 = DB_global::cz_insert('ideation_platform_dtl_2', $_arrayDtl2, true);
                    }
                }
            ...
            if(count($arrAdhoc) > 0){
                $this->UpdateFlag('ideation_platform_dtl_3', $platform_id);
                $paramDel = array(
                    'platform_id' => $platform_id,
                    'flag_active' => 0
                );
                DB_global::cz_delete_array('ideation_platform_dtl_3', $paramDel);
                foreach($arrAdhoc as $adhocDtl){
                    $sql = "SELECT id FROM ideation_platform_dtl_3 WHERE platform_id=:platform_id and imdl_id=:imdl_id";
                    $param = array(
                        'platform_id' => $platform_id,
                        'imdl_id' => $adhocDtl->value,
                    );
                    ...
                    if(!empty($checkUserId)){
                        /* kalo ada, update flag ke 1*/
                        DB_global::cz_update('ideation_platform_dtl_3', 'flag_active', 1, $param);
                    } else {
                        $_arrayDtl4 = array(
                            'platform_id'=>$platform_id,
                            'imdl_id'=>$adhocDtl->value,
                        ...
                        $dtl3 = DB_global::cz_insert('ideation_platform_dtl_3', $_arrayDtl4, true);
                    }
                }
    ...
    public function DeleteData(Request $request)
    {
        $id = $request->input('id');
        try {
            $dtl1 = DB_global::cz_delete('ideation_platform_dtl_1','platform_id',$id);
            $dtl2 = DB_global::cz_delete('ideation_platform_dtl_2','platform_id',$id);
            $dtl3 = DB_global::cz_delete('ideation_platform_dtl_3','platform_id',$id);
    ...
    public function GetPlatformAccess(Request $request)
    {
        ...
        $sql = "SELECT DISTINCT h.* FROM $this->table_name h
                LEFT JOIN ideation_platform_dtl_1 d1 on d1.platform_id = h.id
                LEFT JOIN ideation_platform_dtl_2 d2 on d2.platform_id = h.id
                LEFT JOIN ideation_platform_dtl_3 d3 on d3.platform_id = h.id
    ...
    public function getRoleAdmin(Request $request){
        ...
        try {
            $sql = "select case when group_id = 1 then 'admin' else '' end as role from ideation_platform_dtl_4 where user_id = :user_id and platform_id = :platform_id limit 1";
            $param = [
                'user_id'=>$user_id,
                'platform_id'=>$platform_id
            ];
