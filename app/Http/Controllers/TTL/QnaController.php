...
use DB_global;
use App\Models\TTL\dialogue_yawa_qna;
...
{
    protected $table_name = 'dialogue_yawa_qna';
    protected $folder_name = 'listen/qna';
    ...
    public function ListData(Request $request)
    {
        ...
        try {
            $query = dialogue_yawa_qna::select('*')
    ...
    public function ListAllQna(Request $request)
	{
        $platform_id = $request->input('platform_id');
        $category = $request->input('category');
        ...
        try {
            ...
            if($category != "COUNT"){
                $data = $query->orderBy('sort_index', 'asc')->get();
            } else {
                $data = $query->count();
            }
            return response()->json([
    ...
    public function InsertData(Request $request)
    {
        ...
        unset($_arrayData['user_account'] , $_arrayData['qna_file']);
    ...
    {
        $id = $request->input('id');
        $all = $request->except('user_account','id','qna_file');
        ...
        if($request->hasFile('qna_file'))
        {
            ...
            $fileName = DB_global::cleanFileName($fileName);
            Storage::putFileAs($this->folder_name, $file, $fileName, 'public');
            $all = array_merge($all, ['qna_image' => $fileName]);
        }
