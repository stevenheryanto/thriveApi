...
{
    protected $table_name = 'dialogue_event_schedule';
    protected $folder_name = 'listen/schedule';
    ...
    public function ListData(Request $request)
    {
        ...
        if ($category != "COUNT" && $export == false)
        {
            if(isset($status_active)){
                $sql = "select * from $this->table_name where is_deleted = 0 and platform_id = :platform_id and status_active = $status_active order by title, platform_id asc";
            }
   ...
   public function SelectData(Request $request)
   {
    ...
    public function InsertData(Request $request)
    {
        ...
        unset($_arrayData['user_account'] , $_arrayData['schedule_file']);
    ...
    {
        $id = $request->input('id');
        $all = $request->except('user_account','id','schedule_file');
        ...
        if($request->hasFile('schedule_file'))
        {
            ...
            $fileName = DB_global::cleanFileName($fileName);
            Storage::putFileAs($this->folder_name, $file, $fileName, 'public');
            $all = array_merge($all, ['schedule_image' => $fileName]);
        }
