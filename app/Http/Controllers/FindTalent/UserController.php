...
class UserController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if($config_maintenance->value=='TRUE'){
            ...
            $sql = "
                select 
                    a.*,
                    case when b.group_id = 2 then 'super admin' else '' end as role, 
                    case when first_login is null then 1 when DATEDIFF(now(), last_login) > 30 then 1 else 0 end as flag_show_web_tour 
                from 
                    users a left join group_user b on a.id = b.user_id 
                where 
                    status_active = 1 and (a.email = :a or a.account = :b) 
                    and status_enable = 1 
                limit 1";
        ...
        if(isset($user)){
            $affectedRows = User::whereRaw('status_active = 1 and (email = ? or account = ?) and status_enable = 1', [$credentials,$credentials])->update([
                ...
                'findtalent_first_login' => ($user->findtalent_first_login == null ? $date : $user->findtalent_first_login),
                'findtalent_last_access' => $date
            ]);
        }
    ...
    public function getPlatform(Request $request){
        ...
        $sql = "select DISTINCT a.id,a.name,a.platform_image,a.theme_id, (select case when b.group_id = 1 then 'admin' when b.group_id = 2 then 'super' else '' end as role from group_user b where b.platform_id = a.id and b.user_id = ?) as role from findtalent_platform_hdr a left join findtalent_platform_dtl_1 c on c.id = a.id left join findtalent_platform_dtl_2 d on d.id = a.id where (c.country = ? or d.function =?)";
    ...
    public function ListData(Request $request)
    {
        ...
        $chkCountry = DB_global::cz_result_set('SELECT country FROM findtalent_platform_dtl_1 WHERE platform_id=:platform_id', [$platform_id]);
        $chkDirectorate = DB_global::cz_result_set('SELECT directorate FROM findtalent_platform_dtl_2 WHERE platform_id=:platform_id', [$platform_id]);
        ...
        $adhoc_user = findtalent_platform_dtl_3::select('imdl_id')->from('findtalent_platform_dtl_3')
        ->where([
