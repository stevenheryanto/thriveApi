...
class UserController extends Controller
{
        ...
        if($config_maintenance->value=='TRUE'){
            ...
            $sql = "select a.*,case when b.group_id = 2 then 'super admin' else '' end as role,
                    ...
                    from users a left join
                        timetothink_platform_dtl_4 b on a.id = b.user_id
            ...
            $sql = "select a.*,case when b.group_id = 2 then 'super admin' else '' end as role, case when first_login is null then 1 when DATEDIFF(now(), last_login) > 30 then 1 else 0 end as flag_show_web_tour from users a left join timetothink_platform_dtl_4 b on a.id = b.user_id where status_active = 1 and (a.email = :a or a.account = :b) limit 1";
    ...
    public function ListData(Request $request)
    {
        ...
        $chkCountry = DB_global::cz_result_set('SELECT country FROM timetothink_platform_dtl_1 WHERE platform_id=:platform_id', [$platform_id]);
        $chkDirectorate = DB_global::cz_result_set('SELECT directorate FROM timetothink_platform_dtl_2 WHERE platform_id=:platform_id', [$platform_id]);
    ...
    public function getPlatform(Request $request){
        ...
        $sql = "select DISTINCT a.id,a.name,a.platform_image,a.theme_id, (select case when b.group_id = 1 then 'admin' when b.group_id = 2 then 'super' else '' end as role from timetothink_platform_dtl_4 b where b.platform_id = a.id and b.user_id = ?) as role from timetothink_platform_hdr a left join timetothink_platform_dtl_1 c on c.id = a.id left join timetothink_platform_dtl_2 d on d.id = a.id where (c.country = ? or d.function =?)";
    ...
    public function logout_now(Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Sorry, the user cannot be logged out TTT'
            ], Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
